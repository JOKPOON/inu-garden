            ...
            >
              ...
              >
                ...
                <optgroup
                  v-for="so in sos"
                  :key="so.id"
                  :label="so.code + ' - ' + so.description_eng"
                >
                  <option
                    v-for="sso in so.sub_student_outcomes"
                    :key="sso.id"
                    :value="sso"
                  >
                    {{ sso.code }} - {{ sso.description_eng }}
                  </option>
                </optgroup>
              </select>
            </div>
...
const addSO = () => {
  try {
    ...
    props.ssos.push(selectSO.value);
    emit("updated");
    emit("close");
  } catch (error) {
    console.error(error);
...
onMounted(async () => {
  await fetchSOs(sos, props.program_id);
});
</script>
