<template>
  <div class="shadow-sm rounded-xl p-6 bg-white border border-grey-tertiary flex flex-col gap-6">
    <div class="flex flex-row">
      <div class="border border-grey-secondary rounded-xl">
        <button
          v-for="button in buttons"
          :key="button"
          :class="[ 'px-6 py-3 rounded-xl', activeButton === button ? 'bg-black-primary text-white' : 'border border-grey-secondary text-base' ]"
    ...
    <div v-if="activeButton === 'Overview'">
      <ScoreChart :activeButton="activeButton" />
    </div>
    <div v-else-if="activeButton === 'Grade Overview'">
      <GradeChart :activeButton="activeButton" />
    </div>
  </div>
</template>
