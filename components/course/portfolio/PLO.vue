...
const structuredPlos = computed(() => {
  return Object.values(props.plos).reduce((acc, splo) => {
    const existingPlo = acc.find((p) => p.id === splo.PLOID);
    const formattedCLOs = Object.values(splo.CLOPassingRate).map((clo) => ({
      id: clo.CLOID,
      name: clo.CLOCode,
    ...
    const subPLO = {
      sub_plo: splo.SPLOCode,
      expect: splo.PassedPercentage,
      define: splo.ExpectedPassingCloPercentage,
      CLOPassingRate: formattedCLOs,
    };

    if (existingPlo) {
      existingPlo.sub.push(subPLO);
    } else {
      acc.push({
        id: splo.PLOID,
        plo: splo.PLOCode,
