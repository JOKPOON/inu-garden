<template>
    ...
    >
        <div class="flex gap-4 p-4 border-b font-bold w-full">
          ...
          <div class="flex-6 flex gap-2">
            <div v-for="clo in allCLOs" :key="clo" class="flex-1 flex items-center justify-center">{{ clo }}</div>
          </div>
        </div>
      ...
      >
        <div v-for="student in studentResults" :key="student.studentID" class="flex gap-4 p-4 border-b">
          ...
          <div class="flex-6 flex gap-2">
            <div v-for="clo in allCLOs" :key="clo" class="flex-1 flex items-center justify-center">
                <component :is="student.CLO[clo] ? Include : NotInclude" :class="student.CLO[clo] ? 'text-green-700' : 'text-red-700'" class="w-5 h-5" />
            </div>
          </div>
        </div>
      </div>
  ...
  const allCLOs = Array.from(new Set(studentResults.flatMap(student => Object.keys(student.CLO)))).sort((a, b) => {
